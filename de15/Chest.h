...
public:
	...
	virtual BOOL Use(IDirect3DDevice9* device, CPlayer* player, C2DDraw* pDisplay, IGameMap* pMap);
	BOOL m_bIsOpen;
	e_chest_type m_Type;
	virtual BOOL Create(CPoint p, CSize s, int level, IDirect3DDevice9* device, BOOL bOpen = FALSE, BOOL bKey = FALSE, BOOL bDestroyAfterUse = FALSE);
	CChest();
	virtual ~CChest();
