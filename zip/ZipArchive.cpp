...
bool CZipArchive::OpenFile(LPCTSTR lpszFile)
{
    int index = FindFile(lpszFile);
    if(index == -1)
        return false;
    return OpenFile((WORD)index);
}
...
bool CZipArchive::ExtractFileTo(LPCTSTR lpszSrc, LPCTSTR lpszDest, DWORD nBufSize)
{
    if (!nBufSize)
		return false;

    int iIndex = FindFile(lpszSrc);

    if( iIndex == -1)
        return false;


    WORD uIndex = (WORD)iIndex;

	
	CZipFileHeader header;
	GetFileInfo(header, uIndex); // to ensure that slash and oem conversions take place
	CString szFile = lpszDest;
	if (IsFileDirectory(uIndex))
	{
		ForceDirectory(szFile);
		SetFileAttributes(szFile, header.m_uExternalAttr);
