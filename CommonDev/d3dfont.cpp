...
HRESULT CD3DFont::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
   ...
   hr = pd3dDevice->CreateTexture( m_dwTexWidth, m_dwTexHeight, 1,
      0, D3DFMT_A4R4G4B4,
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DFont::RestoreDeviceObjects(LPDIRECT3DDEVICE9 pd3dDevice)
{
   HRESULT hr;

   if( FAILED( hr = InitDeviceObjects(pd3dDevice )))
   {
      return hr;
   }

   // Create vertex buffer for the letters
   if( FAILED( hr = pd3dDevice->CreateVertexBuffer( MAX_NUM_VERTICES*sizeof(FONT2DVERTEX),
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawTextScaled(LPDIRECT3DDEVICE9 pd3dDevice,  FLOAT x, FLOAT y, FLOAT z,
...
// Desc: Draws 2D text
//-----------------------------------------------------------------------------
HRESULT CD3DFont::DrawText(LPDIRECT3DDEVICE9 pd3dDevice, FLOAT sx, FLOAT sy, DWORD dwColor,
...
//-----------------------------------------------------------------------------
HRESULT CD3DFont::Render3DText(LPDIRECT3DDEVICE9 pd3dDevice,  TCHAR* strText, DWORD dwFlags )
{
   if( pd3dDevice == NULL )
      return E_FAIL;
   ...
   m_dwSavedStateBlock->Apply();
   pd3dDevice->SetVertexShader( NULL );
   pd3dDevice->SetFVF( D3DFVF_FONT3DVERTEX );
   pd3dDevice->SetPixelShader( NULL );
   pd3dDevice->SetStreamSource( 0, m_pVB, 0, sizeof(FONT3DVERTEX) );
