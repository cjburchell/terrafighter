...
#define HALFPI 1.5707963265f

inline FLOAT ToDeg(const FLOAT val)
{
   FLOAT deg = (FLOAT)(val/(PI/180.0f));
   if(deg < 0)
      deg +=360.0;
   return deg;
}

inline double ToRad(const double val)
{
    return (double)(val*PI/180);
}
...
inline void NormalizeAngle(FLOAT& val)
{
   while(val>PI2)
      val -=PI2;	
   while(val<0)
...
namespace D3DUtil
{
   ...
   //-----------------------------------------------------------------------------
   HRESULT CreateTexture( LPDIRECT3DDEVICE9 pd3dDevice,const TCHAR* strTexture,
      CComPtr<IDirect3DTexture9>& pTexture, CZipArchive* pZip,
      D3DFORMAT d3dFormat = D3DFMT_UNKNOWN );

   HRESULT ReleaseTexture(CComPtr<IDirect3DTexture9>& pTexture);
...
public:
   ...
   void SetViewMatrix(D3DXMATRIX *pMat){m_matView = *pMat;};
