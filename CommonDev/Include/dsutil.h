...
public:
   ...
   HRESULT Create( CSound** ppSound, LPCTSTR strWaveFileName, CZipArchive* pZip, DWORD dwCreationFlags = 0, GUID guid3DAlgorithm = GUID_NULL, DWORD dwNumBuffers = 1);
   ...
   HRESULT CreateStreaming( CStreamingSound** ppStreamingSound, LPTSTR strWaveFileName,
      CZipArchive* pZip,DWORD dwCreationFlags, GUID guid3DAlgorithm, DWORD dwNotifyCount, DWORD dwNotifySize, HANDLE hNotifyEvent );
};
...
public:
   DWORD GetCurrentBufferNo(){return m_CurrentBuffer;}
   ...
   HRESULT Play3D(DWORD mode, D3DXVECTOR3 vel, D3DXVECTOR3 pos,FLOAT min = 0.0f, FLOAT max = DS3D_DEFAULTMAXDISTANCE, DWORD dwPriority = 0, DWORD dwFlags = 0 );
...
public:
   ...
   HRESULT Open( LPCTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags , CZipArchive* pZip );
