...
#define PBT_APMRESUMESUSPEND 0x0007
   ...
   case WM_COMMAND:
      ...
      case IDM_TOGGLESTART:
         ...
         DXUtil::Timer( m_bFrameMoving ? DXUtil::TIMER_START : DXUtil::TIMER_STOP );
         break;

      case IDM_SINGLESTEP:
         // Single-step frame movement
         if( FALSE == m_bFrameMoving )
            DXUtil::Timer( DXUtil::TIMER_ADVANCE );
         else
...
#endif
         D3DUtil::SetDeviceCursor( m_pd3dDevice, hCursor, FALSE );
         m_pd3dDevice->ShowCursor( TRUE );
      }
...
HRESULT CD3DApplication::Resize3DEnvironment()
{
   ...
   if( FAILED( hr = DeleteDeviceObjects() ) )
      return hr;
...
INT CD3DApplication::Run()
{
   ...
   while( WM_QUIT != m_msg.message  )
   {
      ...
      if( bGotMsg )
      {
         ...
         if( 0 == TranslateAccelerator( m_hWnd, hAccel, &m_msg ) )
         {
...
HRESULT CD3DApplication::Render3DEnvironment()
{
   ...
   FLOAT fAppTime        = DXUtil::Timer( DXUtil::TIMER_GETAPPTIME );
   FLOAT fElapsedAppTime = DXUtil::Timer( DXUtil::TIMER_GETELAPSEDTIME );
   ...
   // Keep track of the frame count
   {
      ...
      if( fTime - fLastTime > 1.0f )
      {
         ...
         m_fTPS	  = (FLOAT)dwTri / (fTime - fLastTime);
         fLastTime = fTime;
         dwFrames  = 0L;
         dwTri = 0L;
         ...
         m_strFrameStats.Format(_T("%.02f fps %2.2f tps %d tpf (%dx%dx%d)"), m_fFPS,m_fTPS,m_dwTR,
         ...
         if( m_bUseDepthBuffer )
         {
            ...
            {
            case D3DFMT_D16:
               m_strFrameStats+= _T(" (D16)");
               break;
            case D3DFMT_D15S1:
               m_strFrameStats+= _T(" (D15S1)") ;
               break;
            case D3DFMT_D24X8:
               m_strFrameStats+= _T(" (D24X8)") ;
               break;
            case D3DFMT_D24S8:
               m_strFrameStats+= _T(" (D24S8)") ;
               break;
            case D3DFMT_D24X4S4:
               m_strFrameStats+= _T(" (D24X4S4)") ;
               break;
            case D3DFMT_D32:
               m_strFrameStats+= _T(" (D32)") ;
               break;
            }
         }
         ...
         case D3DMULTISAMPLE_2_SAMPLES:
            m_strFrameStats+= _T(" (2x Multisample)");
            break;
         case D3DMULTISAMPLE_3_SAMPLES:
            m_strFrameStats+= _T(" (3x Multisample)");
            break;
         case D3DMULTISAMPLE_4_SAMPLES:
            m_strFrameStats+= _T(" (4x Multisample)");
            break;
         case D3DMULTISAMPLE_5_SAMPLES:
            m_strFrameStats+= _T(" (5x Multisample)");
            break;
         case D3DMULTISAMPLE_6_SAMPLES:
            m_strFrameStats+= _T(" (6x Multisample)");
            break;
         case D3DMULTISAMPLE_7_SAMPLES:
            m_strFrameStats+= _T(" (7x Multisample)");
            break;
         case D3DMULTISAMPLE_8_SAMPLES:
            m_strFrameStats+= _T(" (8x Multisample)");
            break;
         case D3DMULTISAMPLE_9_SAMPLES:
            m_strFrameStats+= _T(" (9x Multisample)");
            break;
         case D3DMULTISAMPLE_10_SAMPLES:
            m_strFrameStats+= _T(" (10x Multisample)");
            break;
         case D3DMULTISAMPLE_11_SAMPLES:
            m_strFrameStats+= _T(" (11x Multisample)");
            break;
         case D3DMULTISAMPLE_12_SAMPLES:
            m_strFrameStats+= _T(" (12x Multisample)");
            break;
         case D3DMULTISAMPLE_13_SAMPLES:
            m_strFrameStats+= _T(" (13x Multisample)");
            break;
         case D3DMULTISAMPLE_14_SAMPLES:
            m_strFrameStats+= _T(" (14x Multisample)");
            break;
         case D3DMULTISAMPLE_15_SAMPLES:
            m_strFrameStats+= _T(" (15x Multisample)");
            break;
         case D3DMULTISAMPLE_16_SAMPLES:
            m_strFrameStats+= _T(" (16x Multisample)");
            break;
         }
      }
   }
