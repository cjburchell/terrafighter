...
HRESULT CMusicManager::Initialize( HWND hWnd, DWORD dwPChannels, DWORD dwDefaultPathType )
{
    ...
    if( FAILED( hr = CoCreateInstance( CLSID_DirectMusicLoader, NULL, CLSCTX_INPROC, 
        ...
        return DEBUG_MSG_HR( TEXT("CoCreateInstance"), hr );
                                                ...
                                                dwPChannels, DMUS_AUDIOF_ALL, NULL ) ) )
    {
        ...
        {
            DEBUG_MSG( "Warning: No sound card found\n" );
            return hr;
        }

        return DEBUG_MSG_HR( TEXT("InitAudio"), hr );
    }
                                              ...
                                              BOOL bIsMidiFile )
{
                                                     ...
                                                     (LPVOID*) &pSegment ) ) )
    {
        if( hr == DMUS_E_LOADER_FAILEDOPEN )
            return hr;
        return DEBUG_MSG_HR( TEXT("LoadObjectFromFile"), hr );
    }
    ...
    *ppSegment = new CMusicSegment( m_pPerformance, m_pLoader, pSegment );
    ...
    if( bIsMidiFile )
    {
        if( FAILED( hr = pSegment->SetParam( GUID_StandardMIDIFile, 
                                             0xFFFFFFFF, 0, 0, NULL ) ) )
            return DEBUG_MSG_HR( TEXT("SetParam"), hr );
    }

    if( bDownloadNow )
    {
        if( FAILED( hr = (*ppSegment)->Download() ) )
            return DEBUG_MSG_HR( TEXT("Download"), hr );
    }
                                             ...
                                             TCHAR* strFileName )
{
    ...
    if ( FAILED( hr = pScript->Init( m_pPerformance, NULL ) ) )
        return DEBUG_MSG_HR( TEXT("Init"), hr );
...
{       
    ...
    if( FAILED( hr = CreateStyleFromFile( &pStyle, strStyle ) ) )
        return DEBUG_MSG_HR( TEXT("CreateStyleFromFile"), hr );
    ...
    if( pStyle )
    {
        ...
        if( FAILED( hr ) )
            return DEBUG_MSG_HR( TEXT("GetMotif"), hr );
