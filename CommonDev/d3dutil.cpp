...
//-----------------------------------------------------------------------------
// Name: D3DUtil::InitMaterial()
// Desc: Initializes a D3DMATERIAL8 structure, setting the diffuse and ambient
//       colors. It does not set emissive or specular colors.
//-----------------------------------------------------------------------------
VOID D3DUtil::InitMaterial( D3DMATERIAL9& mtrl, FLOAT r, FLOAT g, FLOAT b,
                           FLOAT a )
{
...
public:
   ...
   CString m_FileName;
   DWORD m_RefCount;
   BOOL operator == (CTextureCash Other)
   {
      return m_FileName == Other.m_FileName;
   }
};

std::list<CTextureCash> g_TextureCashList;
//-----------------------------------------------------------------------------
// Name: D3DUtil::CreateTexture()
// Desc: Helper function to create a texture. It checks the root path first,
//       then tries the DXSDK media path (as specified in the system registry).
//-----------------------------------------------------------------------------
HRESULT D3DUtil::CreateTexture( LPDIRECT3DDEVICE9 pd3dDevice, const TCHAR* strTexture,
                               CComPtr<IDirect3DTexture9>& pTexture, CZipArchive* pZip,
                               D3DFORMAT d3dFormat )
{
   // Get the path to the texture
   // find a cashed textrue
   for(std::list<CTextureCash>::iterator i = g_TextureCashList.begin();i!=g_TextureCashList.end();i++)
   {
      if(i->m_FileName == strTexture)
      {
         pTexture = i->m_pTexture;
         i->m_RefCount++;
         return S_OK;
      }
   }
   ...
   if(pZip)
   {
      if(pZip->OpenFile(strTexture))
      {

         CZipFileHeader fh;
         pZip->GetFileInfo(fh, pZip->FindFile(strTexture));
         int filesize = fh.m_uUncomprSize;

         BYTE* pBuffer = new BYTE[filesize];
         memset(pBuffer,0,filesize);
         int retval = pZip->ReadFile(pBuffer,filesize);
         if( retval > 0)
         {
            // Create the texture using D3DX
            hr = D3DXCreateTextureFromFileInMemoryEx( pd3dDevice, pBuffer, filesize,
            ...
            {
               CTextureCash Cash;
               Cash.m_FileName = strTexture;
               Cash.m_pTexture = pTexture;
               Cash.m_RefCount = 1;
               g_TextureCashList.push_back(Cash);
            }
         }
         delete [] pBuffer;
   ...
   {
      hr = D3DXCreateTextureFromFileEx( pd3dDevice, strTexture, 
         D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, d3dFormat, 
         D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
         D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &pTexture );
   }
...
HRESULT D3DUtil::ReleaseTexture(CComPtr<IDirect3DTexture9>& pTexture)
{
   if(pTexture)
   {
      for(std::list<CTextureCash>::iterator i = g_TextureCashList.begin();i!=g_TextureCashList.end();i++)
      {
         if(i->m_pTexture == pTexture)
         {
            i->m_RefCount--;
            if(i->m_RefCount == 0)
            {
               CTextureCash Cash = *i;
               //sys_printf(__LOCATION__, 0, "DEBUG: Deleteing %s\n",i->m_FileName);
               i->m_pTexture = NULL;
               g_TextureCashList.remove(Cash);
            }
            return S_OK;
         }
      }
...
//-----------------------------------------------------------------------------
// Name: D3DUtil::SetColorKey()
// Desc: Changes all texels matching the colorkey to transparent, black.
//-----------------------------------------------------------------------------
HRESULT D3DUtil::SetColorKey( LPDIRECT3DTEXTURE9 pTexture, DWORD dwColorKey )
{
...
//-----------------------------------------------------------------------------
// Name: D3DUtil::CreateVertexShader()
// Desc: Assembles and creates a file-based vertex shader
//-----------------------------------------------------------------------------
HRESULT D3DUtil::CreateVertexShader( LPDIRECT3DDEVICE9 pd3dDevice, 
                                    ...
                                    IDirect3DVertexShader9** pVertexShader )
{
   ...
   if( FAILED( hr = D3DXAssembleShaderFromFile( strFilename, 0, NULL, 0, &pCode, NULL ) ) )
      return hr;
...
//-----------------------------------------------------------------------------
// Name: D3DUtil::GetCubeMapViewMatrix()
// Desc: Returns a view matrix for rendering to a face of a cubemap.
//-----------------------------------------------------------------------------
D3DXMATRIX D3DUtil::GetCubeMapViewMatrix( DWORD dwFace )
{
...
//-----------------------------------------------------------------------------
// Name: D3DUtil::GetRotationFromCursor()
// Desc: Returns a quaternion for the rotation implied by the window's cursor
//       position.
//-----------------------------------------------------------------------------
D3DXQUATERNION D3DUtil::GetRotationFromCursor( HWND hWnd,
...
//-----------------------------------------------------------------------------
// Name: D3DUtil::SetDeviceCursor
// Desc: Gives the D3D device a cursor with image and hotspot from hCursor.
//-----------------------------------------------------------------------------
HRESULT D3DUtil::SetDeviceCursor( LPDIRECT3DDEVICE9 pd3dDevice, HCURSOR hCursor,
                                 BOOL bAddWatermark )
{
   HRESULT hr = E_FAIL;
   ICONINFO iconinfo;
...
//-----------------------------------------------------------------------------
inline D3DXQUATERNION* WINAPI _QuaternionUnitAxisToUnitAxis2
...
//-----------------------------------------------------------------------------
inline D3DXQUATERNION* WINAPI _QuaternionAxisToAxis
( D3DXQUATERNION *pOut, const D3DXVECTOR3 *pvFrom, const D3DXVECTOR3 *pvTo)
{
   D3DXVECTOR3 vA, vB;
   ...
   return _QuaternionUnitAxisToUnitAxis2(pOut, &vA, &vHalf);
}
   ...
   case WM_MOUSEMOVE:
      ...
      if( MK_LBUTTON&wParam )
      {
         if( m_bDrag )
         {
            ...
            _QuaternionAxisToAxis(&qAxisToAxis, &s_vDown, &vCur);
            m_qNow = m_qDown;
            m_qNow *= qAxisToAxis;
