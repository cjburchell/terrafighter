...
HRESULT CSessionInfo::RefreshPlayerInfo( DPNID id )
{
   ...
   if( pPlayer )
   {
      ...
      if( strURL )
      {
         DXUtil::ConvertWideStringToGeneric( pPlayer->strURL, strURL, 256 );
      }

      if( pDpPlayerInfo->pwszName)
      {
         DXUtil::ConvertWideStringToGeneric( pPlayer->strName, 
...
HRESULT CSessionInfo::RefreshGroupInfo( DPNID id )
{
   ...
   {
      // Set the group data
      DXUtil::ConvertWideStringToGeneric( pGroup->strName, pDpGroupInfo->pwszName, 256 ); 
   }
   ...
   case DPN_MSGID_APPLICATION_DESC:
      {
         ...
         if( SUCCEEDED( GetDpAppDesc( &pAppDesc ) ) )
         {
            ...
            DXUtil::ConvertWideStringToGeneric( strTitle, 
...
HRESULT CSessionInfo::OnPlayerInfoReceive( SI_MSG_PLAYERINFO* pPlayerInfo )
{
   ...
   DXUtil::ConvertWideStringToGeneric( pPlayer->strName, pStrName, 256 ); 
...
HRESULT CSessionInfo::OnGroupInfoReceive( SI_MSG_GROUPINFO* pGroupInfo )
{
   ...
   // Set updated information
   DXUtil::ConvertWideStringToGeneric( pGroup->strName, pStr, 256 ); 
...
HRESULT CSessionInfo::SendPlayerInfoToPlayer( DPNID idPlayer, DPNID idTarget )
{
   ...
   DXUtil::ConvertGenericStringToWide( pStr, pPlayer->strName, dwNameLen+1 );
...
HRESULT CSessionInfo::SendGroupInfoToPlayer( DPNID idGroup, DPNID idTarget )
{
   ...
   // Pack the strings
   LPWSTR pStr = (LPWSTR) (pMsg+1);
   DXUtil::ConvertGenericStringToWide( pStr, pGroup->strName, dwNameLen+1 );
   ...
   case WM_INITDIALOG:
      {
         ...
         if( SUCCEEDED( GetDpAppDesc( &pAppDesc ) ) )
         {
            ...
            DXUtil::ConvertWideStringToGeneric( strTitle, pAppDesc->pwszSessionName, 256-50 );
            lstrcat( strTitle, TEXT(" - Session Info") );
